...
public class RomanMath extends JFrame implements ActionListener { 
 ...
 String titleString = "Roman Number Converter by Daniel Yang";
 ...
 for (int i=1; i<=numberOfRomanDigits; i++)
 {
  ...
  { 
    ...
    setTitle("Roman Math  by  Daniel Yang");
 ...
 else
 {
  ...
  if (isValidRomanNumeral(romanNumber))
  {
   ...
   String baseEightValue = Integer.toOctalString(baseTenValue);   
  
  
   // FINISH ME
   // find the base sixteen number (Integer class method to?????)
   String baseSixteenValue = Integer.toHexString(baseTenValue);  
